...
# EXTENTION 2: Sample: 30 OECD time 1993 - 2019
# Note: Data is only available for 19 oecd countries
# (therefore, it is probably the same as before)
#=============================================
...
# load data
gni_ext2_raw <- read.csv("../data/data_iy_ext_2.csv")
...
# merge data
merged_df <- list(gni_ext2, ehb_data) %>%
...
stargazer(
  reg_gni_stage2,
  type = "latex",
  out = file.path(output_dir, "ext2_table3_2019_reg_GNI_risk_sharing.tex"),
  title = "OECD ext2 1993 - 2019 GNI Risk Sharing and Equity Home Bias",
...
file.exists(file.path(output_dir, "ext2_table3_2019_reg_GNI_risk_sharing.tex"))
...
stargazer(
  table5_gni_models,
  type = "latex",
  out = file.path(output_dir, "ext2_table5_2019_GNI_foreign_assets.tex"),
  title = "EXT2 OECD REP 1993 - 2019 GNI Risk Sharing and Foreign Asset Holdings Relative to GDP",
  dep.var.labels = "GNI Risk Sharing",
  column.labels = c(
    "Equity",
    "Debt",
...
file.exists(
  file.path(output_dir, "ext2_table5_2019_GNI_foreign_assets.tex")
)
