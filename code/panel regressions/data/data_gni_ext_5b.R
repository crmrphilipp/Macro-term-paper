# ============================================
# Extention 5b: Regression Sample without Euro Zone
# and without EU countries 1993 - 2019
#=============================================
...
# load data
gni_rep_raw <- read.csv("../data/data_iy_ext_5b.csv")

merged_df <- gni_rep_raw

# load home bias data
ehb_data <- read.csv("../data/ehb_ea_cap_small.csv")
ehb_df <- ehb_df %>%
  mutate(iso = ifelse(iso == "EUR", "EA20", iso))
...
reg_sample <- c(
  ...
  "DNK", "ESP", "FRA", "GBR", "GRC", "HKG",
  "HRV", "HUN", "IDN", "IND", "ISR", "ITA",
  "JPN", "KOR", "LUX", "LVA", "MEX", "NLD",
  "NOR", "POL", "PRT", "ROU", "RUS", "SAU",
  "SGP", "SVK", "SVN", "TUR", "USA", "ZAF"
)
