...
# load data
gni_ext1_raw <- read.csv("../data/data_iy_ext_1.csv")
...
# merge data
merged_df <- list(gni_ext1, ehb_data) %>%
...
stargazer(
  reg_gni_stage2,
  type = "latex",
  out = file.path(output_dir, "table3_2019_reg_GNI_risk_sharing.tex"),
  title = "OECD REP 1993 - 2019 GNI Risk Sharing and Equity Home Bias",
...
file.exists(file.path(output_dir, "table3_2019_reg_GNI_risk_sharing.tex"))
...
stargazer(
  table5_gni_models,
  type = "latex",
  out = file.path(output_dir, "table5_2019_GNI_foreign_assets.tex"),
  title = "OECD REP 1993 - 2019 GNI Risk Sharing and Foreign Asset Holdings Relative to GDP",
  dep.var.labels = "GNI Risk Sharing",
  column.labels = c(
    "Equity",
    "Debt",
...
file.exists(
  file.path(output_dir, "table5_2019_GNI_foreign_assets.tex")
)
